...
void MapReduce::master_wait_tasks_mapper() {
	...
	while ((temp = read(mapper_to_master_pipe[0], buff, 1)) > 0) {
		cout << "Master \n";
		completed_tasks++;
...
void MapReduce::master_group_by_keys() {
	string input_file_prefix = "processed_map_";
	...
	string output_file_prefix = "group_";
...
void MapReduce::map(const string& fname) {
...
void MapReduce::reducer(char *fname) {
	reducer_file(fname);
}
...
// User related private functions.
void MapReduce::user_routine() {
